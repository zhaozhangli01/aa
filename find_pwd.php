      ...
      <div id=tagContent>
        <div class="tagContent selectTag tab_1" id=tagContent0>
            ...
            <ul>
              <li>
                  <span>手&nbsp;&nbsp;&nbsp;&nbsp;机&nbsp;：</span>
                  <input name="txtTel" type="text" id="user_phone" class="kuang" placeholder="请输入手机号码">
              </li>
              <li>
                  <span>密&nbsp;&nbsp;&nbsp;&nbsp;码&nbsp;：</span>
          ...
          <div class="hd_register_black">
              <input type="button" name="btnLogin" value="重置" id="btnLogin" class="hd_register_btn hd_register_btnleft" style="margin-bottom: 50px;background:#ddd;" disabled="disabled">
          </div>
...
<script type="text/javascript">
    var login = {
              ...
              .on('click', '#btnLogin', function (e) {
                  if (isLogin_1 != true || isLogin_2 != true || isLogin_3 != true || isLogin_4 != true) {
                    if (isLogin_1 != true) {
                      ...
                      /*登录 ajax 函数*/
                      $(this).removeAttr('btnLogin').css('background', '#ddd').val('重置中...').attr('disabled', 'true');
        ...
        check_phone(data) {
            ...
            $.ajax({
              ...
              success: function (res) {
                if (res.code == 0) {
                  $('#error_notice').html("该账号未注册，或者账号异常，请联系管理员");
                  return;
                }
        ...
        GetVerificationCode(data) {
            ...
            $.ajax({
              type: 'post',
              url: '../index.php/system/login/verify_send_sms_findpwd.html',
              data: data,
              success: function (list) {
                if (list.code == 0) {
                  if (list.msg == '图形验证码不对') {
        ...
        AllVerification(alldata) {
            ...
            $.ajax({
              type: 'post',
              url: '../index.php/system/regist/findpwd.html',
              data: alldata,
              success: function (list) {
                if (list.code == 1) {
                    alert("找回密码成功");
                    ...
                    $('#btnLogin').css('background', '#458de5').val('重置').removeAttr('disabled');
                    if (list.msg.sms_num) {
